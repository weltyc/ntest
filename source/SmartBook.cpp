...
void CSmartBook::PlayEdmundGame(const CQPosition& pos, CMoveValue& mv) {
	char sBoard[NN+1];
	CGame game(m_pComputer, m_pComputer);
	...
	if (bd->IsBranch()) {
		...
		while (1) {
			...
			if (!movesNonbook.NMoves()) {
				...
				abortOnInput=false;
				IterativeValue(movesNonbook, cp, si, mvk, false, 1);
				abortOnInput=true;
				nSearches++;
			}
		}
	...
	for (iMove=nMoves; iMove>=0; iMove--) {
		...
		if (pos.NEmpty()>=NEmptyMin() && pos.CalcMoves(moves)) {
			...
			if (!bd) {
				...
				abortOnInput=false;
				IterativeValue(moves, *(m_pComputer->pcp), si, mvk, false, 1);
				abortOnInput=true;
				nSearches++;
				bd=FindNonconstData(pos.BitBoard());
				QSSERT(bd);
...
void CSmartBook::IncreaseHeight(CQPosition pos, CBookData* bd, CHeightInfo hi, int& nSearches){
	...
	// Uleaf?
		...
		abortOnInput=false;
		IterativeValue(moves, cp, si, mvk, false, 1);
		abortOnInput=true;
		nSearches++;
