...
u1 *configToPotMob[2][9], *configToMob[2][9];
u1 *configToPotMobTriangle[2], *configToMobTriangle[2];
...
void InitLCoeffStarts() {
	int map;

	// calculate nTotalCoeffs and coeffStarts
	for (map=0, nCoeffsL=0; map<nMapsL; map++) {
		coeffStartsL[map]=nCoeffsL;
		nCoeffsL+=mapsL[map].NConfigs();
	}
}
...
int PotMob(u1 black, u1 empty, int length) {
	...
	mask=(1<<length)-1;
//	mask=(1<<8)-1; old buggy version
	empty &= mask;
	mask--;
	return CountBitsInline(black&(empty<<1)&(mask>>1))+CountBitsInline(black&(empty>>1)&mask);
}

int Mob(u1 black, u1 empty, int length) {
	int col;
	u1 mask, a, b, result;
	...
	if (!fInitConfigToPotMob) {
		...
		for (length=3, nConfigs=27; length<=N; length++, nConfigs*=3) {
			for (color=0; color<2; color++) {
				...
				configToMob[color][length]=new u1[nConfigs];
				CHECKNEW(configToPotMob[color][length] && configToMob[color][length]);
			}
		...
		configToMobTriangle[0]=new u1[nConfigs];
		configToMobTriangle[1]=new u1[nConfigs];
		CHECKNEW(configToMobTriangle[0] && configToMobTriangle[1]);
		for (config=0; config<nConfigs; config++) {
	...
	if (fInitConfigToPotMob) {
		...
		for (color=0; color<2; color++) {
			for (length=3; length<=8; length++) {
				...
				delete[] configToMob[color][length];
			}
			delete[] configToPotMobTriangle[color];
			delete[] configToMobTriangle[color];
		}
