...
static int HashWrite(const void* data, size_t size, FILE* fp) {
	HashChunk(data, size);
...
static int HashRead(void* data, size_t size, FILE* fp) {
	const size_t result=fread(data, size, 1, fp);
	if (result)
		HashChunk(data, size);
	return result;
}
...
void CBook::ReadErr() {
	_ASSERT(0);
	throw std::string("WARNING: BOOK ")+m_bookname+" is damaged, restore a backup.\n";
	_exit(-3);
}
...
CBook::CBook(): m_nHashErr(0), m_fAltered(false), m_tLastWrite(0), m_nEmptyMin(hSolverStart+1) {
}
...
//! \param os std::ostream for warning messages.
//! \post comments and errors will be written to os
CBook::CBook(const char* filename, std::ostream& os) : m_nHashErr(0), m_fAltered(false), m_tLastWrite(0), m_nEmptyMin(hSolverStart+1) {
	CBookData bd;
	CBitBoard board;
	FILE* fp;
	...
	if (filename) {
		...
		if (!fp) {
			switch(errno) {
			case ENOENT:
				os << "WARNING: Book " << filename << " doesn't exist, will be created when saving book\n";
				break;
			default:
				os << "WARNING: Book " << filename << " is unavailable, book will start out empty and will not be saved. (errno " << errno << ")\n";
				filename=0;
			}
		}
		else {
			if (fread(&m_iBookFormat, sizeof(m_iBookFormat), 1, fp)==0) {
				os << "WARNING: Book " << filename << " is empty\n";
			}
			else {
				os << "Loading book " << filename << " (format " << m_iBookFormat << ")...";

				switch(m_iBookFormat) {
					case 1: ReadVersion1(fp); break;
					case 2: ReadVersion2(fp); break;
	...
	if (filename) {
		...
		m_bookname.erase(0,-1);
	std::cout << "Done loading book " << filename << std::endl;
}

//! Destroy the book. If the book has a file associated, write a message to cerr.
//!
//! If the book has no name it's probably being used for testing purposes, so don't clutter the screen up.
CBook::~CBook() {
	if (!m_bookname.empty())
		cerr << "Closing book " << m_bookname << "...";
	Write();
	if (!m_bookname.empty())
		cerr << "Done\n";
}
...
void CBook::Write() {
	...
	if (m_fAltered || m_iBookFormat!=s_iBookWriteFormat) {
		std::cerr << "Writing book\n";
		switch(s_iBookWriteFormat) {
			case 2:
				WriteVersion2(m_bookname);
				break;
			case 1:
			default:
				WriteVersion1();
		...
		std::cerr << "Not writing, not altered\n";
	}
}

void CBook::WriteErr() const {
	cerr << "WARNING: Error writing to book file " << m_bookname << " (errno " << errno << ")\n";
}
...
void CBook::WriteVersion1() {
	...
	fp=fopen(m_bookname.c_str(),"wb");
	if (!fp) {
		cerr << "WARNING: Error opening book file " << m_bookname << " for writing (errno " << errno << ")\n";
		return;
	}
	...
	if (!fwrite(&nVersion, sizeof(nVersion), 1, fp) || !fwrite(&nSize, sizeof(nSize), 1, fp)) {
		...
		for (nEmpties=nEmptyBookMax-1; nEmpties>=0; nEmpties--) {
			for (i=entries[nEmpties].begin(); i!=entries[nEmpties].end(); i++) {
				if (!(HashWrite(&((*i).first), sizeof(CBitBoard), fp) &&
					HashWrite(&((*i).second), sizeof(CBookData), fp))) {
					WriteErr();
					fclose(fp);
					return;
				}
...
void CBook::ReadVersion1(FILE* fp) {
	...
	for (nRead=0; nRead<nSize && HashRead(&board, sizeof(board), fp) && HashRead(&bd, sizeof(bd), fp); nRead++) {
...
void CBook::WriteTree2(FILE* fp, const CMinimalReflection& mr, CBookData& bd) {
	CBookDataCompressed bdc(bd);
	HashWrite(&bdc, sizeof(bdc), fp);
	bd.SetWritten();
...
//!
//! \post book is written to file or an error message is written to cerr.
...
void CBook::WriteVersion2(const std::string& fn) {
	...
	if (loc==-1)
		...
		tempDir.assign(fn, 0, loc);
	char fnTemp[MAX_PATH+1];
	GetTempFileName(tempDir.c_str(), "bk", 0, fnTemp);
	fnTemp[MAX_PATH]=0;

	fp=fopen(fnTemp,"wb");
	if (!fp) {
		cerr << "WARNING: Error opening book file " << fnTemp << " for writing (errno " << errno << ")\n";
		return;
	}
		...
		for (nEmpties=nEmptyBookMax-1; nEmpties>=0; nEmpties--) {
			for (i=entries[nEmpties].begin(); i!=entries[nEmpties].end(); i++) {
				if (!i->second.GetWritten()) {
					...
					HashWrite(&mr, sizeof(CBitBoard), fp);
					WriteTree2(fp, mr, i->second);
				}
			}
		}
	}
	...
	if (!MoveFileEx(fnTemp, fn.c_str(), MOVEFILE_REPLACE_EXISTING)) {
		int err=GetLastError();
		std::ostringstream os;
		os << "WARNING: Can't save book file " << fn << "\n";
		switch(err) {
		...
		os << "\n";
		std::cerr << os;
		::MessageBox(NULL, os.str().c_str(), "NTest Book Write Error", MB_OK | MB_ICONWARNING);
		return;
	}
...
//! \param bd [in] Reference to the book data for this position
void CBook::ReadTree2(FILE* fp, const CMinimalReflection& mr) {
	_ASSERT(!mr.IsImpossible());
	CQPosition pos(mr, true);

	// read value
	CBookDataCompressed bdc;
	HashRead(&bdc, sizeof(bdc), fp);
	...
	// read in subtrees.
	char c;
	while (HashRead(&c, 1, fp) && c!=-1) {
		_ASSERT(!pos.BitBoard().IsImpossible());
		CMove move(c);
		CQPosition subpos(pos);
		subpos.MakeMove(move);
		_ASSERT(!subpos.BitBoard().IsImpossible());
		ReadTree2(fp, subpos.BitBoard());
	}
}

void CBook::ReadVersion2(FILE* fp) {
	CBitBoard board;
	CBookDataCompressed bdc;
	...
	CBitBoard bb;
	while (Size()<nSize && HashRead(&bb, sizeof(bb), fp)==1) {
...
const CBookData* CBook::FindData(const CMinimalReflection& mr) const {
	return FindData(mr, CountBits(mr.empty));
}

CBookData* CBook::FindNonconstData(const CMinimalReflection& mr) {
	return FindNonconstData(mr, CountBits(mr.empty));
}
...
bool CBook::Load(const CMinimalReflection& mr, CHeightInfo hi, CValue alpha, CValue beta, CValue& value) const {
	return Load(mr, hi, alpha, beta, value, CountBits(mr.empty));
}
