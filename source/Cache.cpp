...
//! Test that all proven solves in the cache are correct
void CCache::Verify(CPlayerComputer* pcp) {
	const CCacheData *pcdEnd=buckets+nBuckets;
	for (CCacheData *pcd=buckets; pcd < pcdEnd; pcd++) {
		pcd->Verify(pcp);
	}
}

//! Test that all proven solves in the cache are correct
void CCacheData::Verify(CPlayerComputer* pcp) {
	if (height+hSolverStart==Board().NEmpty() && iPrune==0) {
		...
		_ASSERT(bb.empty.i8s!=0x00030103030000c0i64 || bb.mover.i8s!=0x00a4dea090e9c000i64);

		ValueTree(height, lBound-kStoneValue, uBound+kStoneValue, moves, iffCache, 0, mvBest);
		if (mvBest.value<lBound || mvBest.value>uBound) {
			printf("LBound: %d, UBound: %d, mvBest: %d\n", lBound, uBound, mvBest.value);
			Board().FPrint(stdout, true);
			TEST(0);
			bb.CalcMoves(moves);
			iffCache=0;
			mvBest.move=0;
