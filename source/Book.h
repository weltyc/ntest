...
public:
	int pass;
	const CBookData* sbd;
	void Out(ostream& os, u4 iLevel, bool fBlackMove) const;
};
...
public:
	...
	std::string Filename() const { return m_bookname; };
...
private:
	...
	void EndgameError(const CQPosition& pos, const CBookData* bd, const CBookValue& bv) const;

	void WriteErr() const;
	void Write();
	void WriteVersion1();
	void WriteVersion2(const std::string& fn);
	void WriteTree2(FILE* fp, const CMinimalReflection& mr, CBookData& bd);

	void ReadErr();
	void ReadVersion1(FILE* fp);
	void ReadVersion2(FILE* fp);
	void ReadTree2(FILE* fp, const CMinimalReflection& mr);
};
